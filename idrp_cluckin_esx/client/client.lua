...
RegisterNetEvent('idrp_cluckin:UseItem', function(itemName)
				...
				if IsPedOnFoot(ped) then
					if IsAnimated == false then
						...
						if v.type == 'drink' then
							...
							local cluckinItem = CreateObject(GetHashKey(Prop_name), x, y, z + 0.2, true, true, true)
							AttachEntityToEntity(cluckinItem, ped, GetPedBoneIndex(ped, v.bone), v.x, v.y, v.z, v.xr, v.yr, v.zr, true, true, false, true, 1, true)
							Citizen.Wait(14000)
							IsAnimated = false
							DeleteObject(cluckinItem)
						...
						elseif v.type == 'plate' then
							...
							local cluckinItem = CreateObject(GetHashKey(Prop_name), x, y, z + -0.2, true, true, true)
							--left hand plate
							local cluckinItem2 = CreateObject(GetHashKey(Prop_name2), x, y, z+ -0.2, true, true, true)
							--right hand single item
							AttachEntityToEntity(cluckinItem, ped, GetPedBoneIndex(ped, v.bone2), v.x2, v.y2, v.z2, v.xr2, v.yr2, v.zr2, true, true, false, true, 1, true)
							--left hand plate
							AttachEntityToEntity(cluckinItem2, ped, GetPedBoneIndex(ped, v.bone), v.x, v.y, v.z, v.xr, v.yr, v.zr, true, true, false, true, 1, true)
							RequestModel(Prop_name)
